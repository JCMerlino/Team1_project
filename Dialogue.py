stage_1 = {

	"Jenifer":
	"""Jenifer : Hey Bruce, Samantha wants to see you.

	Bruce : Oh wow what a suprise, my boss wants to see
	me. It's not like I work for her or anything. What
	does the old hag want?

	Jenifer : I don't know, I'm not exactly psychic.
	Besides she won't tell me anyway. She doesn't tell
	me anything that goes on in this place.

	Bruce : Well maybe if you did some work from time
	to time she might tell you things to do with, you
	know, work.

	Jenifer : I'm still getting paid either way so I'll
	just stick with doing nothing whilst you and the
	others can work your asses off.

	Bruce : Well thanks for the motivation. I'll go
	talk to Samantha and try not to think of people or
	inanimate objects that could replace you.

	Jenifer : Rude""",



	"JeniferRepeat":
	"""Jenifer : Go speak to Samantha before she gets 
	mad and we both get in trouble.""",



	"Leaving":
	"""Jenifer : Where are you going? I told you
	Samantha wants to speak to you! Do you not have
	ears?

	Bruce : I have ears, I just have selective hearing.

	Jenifer : Go speak to her will you? She'll keep
	pestering me if I don't get you to speak to her.

	Bruce : Gosh I guess this is an important thing
	No wonder shes's asking for...

	Jenifer : Don't say it!

	Bruce : The best spy in the world!

	Jenifer : You sound like a hyperactive five year
	old when you say that.

	Bruce : Does it look like I care? It's a cool
	title. I'll go speak to Samantha because
	obviously she needs me.""",

	"Completion": False
}

stage_2 = {

	"SamanthaStart":
	"""Samantha : Bruce! What time do you call this?

	Bruce : Time you got a watch.

	Samatha : I call it two hours too late. Anyway
	following your most recent field report all
	agents except you and Alexa are in the field.
	We can't assign anything until new intel is
	acquired.

	Bruce : So basically you're telling me I have
	the day off.

	Samantha : No I'm not giving you a day off.
	Debra and Bob have both reported that their
	computers have been acting up recently and I
	need Stephen to take a look at the mainframe.

	Bruce : My job has been downgraded from spy
	to messenger?

	Samantha : Look, make yourself useful and
	help Stephen investigate what's going on with
	the mainframe and see if you can fix it.

	Bruce : Oh, so my job has been downgraded to
	IT help technician instead, noted.

	Samatha : I'm already paying you enough so go do
	something productive!

	Bruce : OK ok. Got it""",



	"SamanthaRepeat":
	"""Samantha : Well don't stand there staring at
	me! Go and help Stephen before I decide you need
	permenant days off""",



	"Jenifer":
	"""Jenifer : So what did she want?

	Bruce : She wants me to help Stephen look at the
	mainframe.

	Jenifer : Riiiiight, so she's given you a task
	just to keep you out of trouble.

	Bruce : ...""",



	"JeniferRepeat":
	"""Jenifer : Don't you have some important thing
	with Stephen to do?""",



	"Debra":
	"""Bruce : Hey Debra, do you mind passing me some
	biscuits?

	Debra : Only if you don't mind replacing them...

	Bruce : Oh come on! There's only one left? You're
	seriously defeating the point of the rules.

	Debra : I don't have to replace them if I don't
	finish them.

	Bruce : Well you might as well replace them
	because you're the one whos always eating them.""",



	"DebraRepeat":
	"""Debra : So you gunna buy me more biscuits or
	what?

	Bruce : No thanks.

	Debra : Suit yourself.""",



	"Bob":
	"""Bob : Oh hi Bruce, what brings you down here?
	Do you need some paperwork?

	Bruce : No Bob. I'll be honest, Samantha has sent
	me down to help Stephen look at the mainframe or
	something.

	Bob : Oh ok. Sounds interesting.

	Bruce : Interesting is being out in the field
	on a do or die mission with lives on the line.
	This is just IT support.

	Bob : Could be worse. You could have my job...

	Bruce : Very true. I'll leave you to get back to
	your filing and other important things.

	Bob : *sighs* Thanks...""",



	"BobRepeat":
	"""Bob : You done helping Stephen yet?

	Bruce : Uuuuuh, no. I'll get back to that...""",



	"Alexa":
	"""Bruce : So how come you're not out in the field?

	Alexa : My last mission gained high heat. I've been
	told to lay low until my profile has lost it.

	Bruce : Wah Wah someone might recognise me in the
	field. Got anymore lame excuses?

	Alexa : You're so immature. No wonder Samantha is
	hesitant to put you in the field.

	Bruce : I'm not going to argue with the maturity
	but I'll have you know I'm on an important internal
	assignment that the organisation depends upon.

	Alexa : ... Really?

	Bruce : Yes. I'm helping Stephen fix the mainframe

	Alexa : Oh please, Stephen could fix it without you.
	Samantha is just making him babysit you.

	Bruce : I don't need babysitting!

	Alexa : Whatever you say hun. Whatever you say...""",



	"AlexaRepeat":
	"""Alexa : Excuse me sir can you help me with my
	phone?

	Bruce : I'm not an IT technician Alexa. Quit it.""",

	"Completion": False
}

stage_3 = {

	"StephenStart":
	"""Bruce : Hey Stephen, Samantha told me to come
	help you.

	Stephen : Really!? Great! Come over here and put
	this on your head.

	Bruce : NO STEPHEN! Last time I put that on I 
	woke up in a shower hugging a chicken. Besides 
	Samantha said you have to take a look at the 
	mainframe because its faulty or something and I have
	to help.

	Stephen : Can't you see I'm a little busy at the
	moment?

	Bruce : Yeah, busy messing with people's brains.

	Stephen : Well I'm busy so I can't help right now

	Bruce : Aw come on. Samantha's going to go crazy
	if we don't fix this. She was really tense when I
	spoke to her.

	Stephen : Well, I do have something I could do. I
	created an executable and stored it on a USB for
	a situation like this.

	Bruce : A situation when the mainframe acts up?

	Stephen : Yes. The USB should fix the problem.
	The executable is prepared for pretty much every
	possible problem the mainframe could be having.
	All you need to do is log on and run it. But
	don't use your login incase it doesn't work and
	it screws it up. That way the logs don't show it
	was you.

	Bruce : So... can I have your login details?

	Stephen : Why would I give you mine? Just go
	find someone else's details and bring back
	whatever the mainframe prints out after the
	executable fixes it.

	Bruce : Got it.""",



	"StephenRepeat":
	"""Stephen : Not now Bruce, just bring me back the
	printed report when you get it.""",



	"AlexaStart":
	"""Bruce : Do you know where the key to the
	mainframe is kept?

	Alexa : Why would I know? Why didn't you ask
	Stephen?

	Bruce : I tried but he was too busy toying with 
	people's brains.

	Alexa : Oh God not again.

	Bruce : Anyway I need the dam key to get into the
	mainframe room. You seen it anywhere?

	Alexa : Now that I think about it, I think I saw
	something like a key at the bottom of the toilet
	bowl when I went earlier.

	Bruce : You've been eating keys? I didn't know
	that was necessary for a healthy diet.

	Alexa : No I didn't eat the key. I just think I
	saw it there. Maybe someone dropped it.

	Bruce : Or maybe someone is supplementing their
	nutritional intake with some keys.

	Alexa : Well that's all I got on the whereabouts
	of the key. Don't say I didn't help.

	Bruce : It wasn't much help. You've just told me
	to go fishing in the toilet.

	Alexa : So go do it. Maybe you'll make some
	friends with the turds.

	Bruce : ...""",



	"AlexaRepeat1":
	"""Alexa : How's turd fishing? Made any friends?""",



	"AlexaFound":
	"""Bruce : Alexa you were right! The key was in
	the toilet bowl. I almost hurled trying to fish it
	out

	Alexa : Yeah I did say.

	Bruce : Well thanks but I do feel quite queesy now.
	What idiot would drop this key in the toilet?

	Alexa : What idiot would fish something out of the
	toilet with his hands instead of using some cooking
	tongs from the kitchen?

	Bruce : That help would have been better earlier.

	Alexa : I thought you'd be smarter.

	Bruce : ...""",



	"AlexaRepeat2":
	"""Alexa : Quit procrastinating by talking to me
	and go finish your business with Stephen.""",



	"BobStart":
	"""Bob : Hey Bruce sorry, I'm a little busy right
	now. I needed the space to lay out the documents so
	I'm gunna be in here instead of at my desk for a
	while. Do you mind if I get on with this?

	Bruce : No problem.""",



	"BobRepeat":
	"""Bob : You done helping Stephen yet?

	Bruce : Uuuuuh, no. I'll get back to that...""",



	"DebraRepeat":
	"""Debra : So you gunna buy me more biscuits or
	what?

	Bruce : No thanks.

	Debra : Suit yourself.""",



	"JeniferRepeat":
	"""Jenifer : Don't you have some important thing
	with Stephen to do?""",



	"SamanthaRepeat":
	"""Samantha : Well don't stand there staring at
	me! Go and help Stephen before I decide you need
	permenant days off""",

	"Completion": False
}

stage_3_1 = {"Completion": False}

stage_3_2 = {

	"JeniferStart":
	"""Bruce : The printer is out of ink.

	Jenifer : Um. Ok?

	Bruce : ... I said, the printer is out of ink.

	Jenifer : So what do you expect me to do about it?

	Bruce : Uh I expected you to get up and replace
	the ink cartridge in the printer.

	Jenifer : That's not in my job description.

	Bruce : Neither is sitting at your desk and doing
	nothing all day.

	Jenifer : Well I'm not doing it so you do it.

	Bruce : Fine. I'll do it.""",



	"JeniferRepeat":
	"""Jenifer : Don't you have some important thing
	with Stephen to do?""",

	"Completion": False
}

stage_3_3 = {"Completion": False}

stage_3_4 = {"Completion": False}


stage_4Coffee = {

	"StephenStart":
	"""Stephen : Ah Bruce. Thank you for the documents.
	I'll have a look at those later. Right now I'm in
	the middle of an experiment.

	Bruce : I'd kind of like you to take a look at them
	now. Otherwise Samantha will be on our asses about
	it.

	Stephen : Don't worry the USB will have done it's
	job and everything should be fine.

	Bruce : Can you have a look at it now please. I
	want to know if I can spend the rest of the day in
	the break room.

	Stephen : Fine I'll have a look at it. Pass it here.
	Hmmm all seems fine. Wait. There's an anonymous
	entry in the vault records under "technology".

	Bruce : Aren't all entries logged with a name and
	time?

	Stephen : Yes. All logs have to conform to a
	standard I set. This log is impossible. Bruce, you
	need to go and fetch me that piece of unknown tech.

	Bruce : Nope. No. Can. Do. I was told to help fix
	the mainframe which is now fixed so I can now do
	nothing and get paid.

	Stephen : Do you think Samantha would be pleased
	to know what you've been using the organisation
	resources to do. I have your search history.

	Bruce : I'll get you that tech as if my life
	depends on it.

	Stephen : You better do. Because judging by
	the categories you search up, your life does
	depend on it. Also, try not to drag other people
	into this. It may seem insignificant but it
	could be rather serious.""",



	"StephenRepeat":
	"""Stephen : Have you got the tech yet?""",



	"JeniferRepeat":
	"""Jenifer : Don't you have some important thing
	with Stephen to do?""",



	"SamanthaStart":
	"""Samantha : Bruce, an update on the situation
	with the mainframe?

	Bruce : You'll be pleased to know that Stephen and
	I have investigated the problem and we are sorting
	it out right now.

	Samantha : You seem to be taking something seriously
	for once. I'm pleased to see that. Anything else
	you need to report?

	Bruce : I fished the keys to the mainframe and
	vault out of the toilet. Does that count?

	Samantha : You can't even stay serious for more
	than five minutes?? Just get out of my office and
	get on with the job!

	Bruce : ...""",



	"SamanthaRepeat":
	"""Samantha : Go get the job finished! And take it
	seriously or I'll cut your pay!""",



	"DebraStart":
	"""Bruce : Do you ever leave the break room?

	Debra : Not if I can help it.

	Bruce : You are unbelievable. Do you actually work?

	Debra : Yes I do. The big pile of paperwork that Bob
	has is thanks to me completing all my paperwork so
	quickly that he has to file it away.

	Bruce : I almost want to believe its true.

	Debra : Don't you have something to do?

	Bruce : Yeah turns out I have work to do. Something
	you and Jenifer don't have the pleasure of doing.""",



	"DebraRepeat":
	"""Debra : So you finished your work yet?

	Bruce : Nope. It's long and difficult and above your
	paygrade.""",



	"BobStart":
	"""Bruce : So someone left the vault password at the
	mainframe. Do you know who last used the mainframe?

	Bob : I don't really know who uses the mainframe
	sorry.

	Bruce : I thought you kept all the logs in the
	organisation?

	Bob : Yeah but not electronic logs. You better ask
	Stephen for those.

	Bruce : Oh yeah... I knew that.""",



	"BobRepeat":
	"""Bob : Have you asked Stephen about those logs yet?

	Bruce : He's quite busy right now. Besides I have
	something else to do.""",



	"AlexaStart":
	"""Bruce : When was the last time you put something
	in the vault?

	Alexa : A long time ago. Why'd you ask?

	Bruce : Oh nothing. It's just I overheard something
	about the vault and was wondering if people knew
	anything about it.

	Alexa : Give me a heads up if you find out anything
	else about the vault.

	Bruce : Will do.""",



	"AlexaRepeat":
	"""Alexa : Any info on the vault?

	Bruce : Nope. I'll get back to you when I find
	something.""",

	"Completion": False
}

stage_4_1C = {"Completion": False}
stage_4_2C = {"Completion": False}

stage_5Coffee = {

	"StephenStart":
	"""Stephen : Now this is interesting...

	Bruce : So you know what it is?

	Stephen : Yes I do and I know what to do with it.
	I need a power plug, a heat plate and some mugs.

	Bruce : Why do you need those?

	Stephen : I'll explain later. Just make sure that
	the mugs are ceramic. It's important.

	Bruce : Can I at least know what you're going to
	do with those items?

	Stephen : I'm going to fix one of the machines
	that has been broken for a while.

	Bruce : Ok that sounds reasonable but why do I
	have to go and fetch them?

	Stephen : Because you'll get to use the machine
	when you need to.

	Bruce : Fair enough. I'll get you the items.""",



	"StephenRepeat":
	"""Stephen : Have you got all the items yet?

	Bruce : No but I'll keep searching.

	Stephen : Good. Come back as soon as you have
	them.""",



	"SamanthaStart":
	"""Samantha : What is it now?

	Bruce : Just popping in to say that the fixing
	of the mainframe is going very well.

	Samantha : Ok. Stop wasting my time and get
	back to it.""",



	"SamanthaRepeat":
	"""Samantha : Stop wasting my time and get back
	to fixing the mainframe.""",



	"JeniferStart":
	"""Bruce : Do you have any idea where I'd find
	a spare power plug?

	Jenifer : Well if you were smart you'd take one
	from a computer whose owner does no work. That
	way it wouldnt be missed.

	Bruce : In that case can I have your power plug?

	Jenifer : Of course not. I'm watching Netflix.

	Bruce : That's not work.

	Jenifer : But I will miss the power plug.

	Bruce : Fine, I'll find another one.""",



	"JeniferRepeat":
	"""Jenifer : How's it going errand boy?.""",



	"DebraStart":
	"""Debra : You've visited this room so many
	times I'm starting to think you don't do any
	work.

	Bruce : Excuse me. I've come to this room on
	work related terms.

	Debra : Really?

	Bruce : Yeah I came to take some mugs.

	Debra : "Work related" he says.

	Bruce : Yes. Stephen needs them for something
	he's fixing.

	Debra : Well whatever you do don't take my mug.

	Bruce : Don't worry I won't.""",



	"DebraRepeat":
	"""Debra : You better not have taken my mug.""",



	"BobStart":
	"""Bob : Hi Bruce hows your day going?

	Bruce : Well it's been pretty underwhelming so
	far. I've done all this stuff and I don't really
	see much outcome for it.

	Bob : Sometimes you have to wait till the end
	to see what you've really worked for.

	Bruce : That's stupid. I should know what I'm
	working for from the start.

	Bob : So you don't know what you've been
	working for today?

	Bruce : Nope not a clue. I just know I have to
	collect some stuff for Stephen now.

	Bob : Do you need help?

	Bruce : I don't know where to get a heat plate
	from.

	Bob : Why don't you try storage?

	Bruce : Yeah why didn't I think of that?""",



	"BobRepeat":
	"""Bob : You finished helping Stephen yet?

	Bruce : Nope.""",



	"AlexaStart":
	"""Bruce : Hey Alexa do you who's mugs are
	easiest to steal?

	Alexa : ...??

	Bruce : It's not a joke it's for Stephen.

	Alexa : ...??

	Bruce : Ok well I'm sure Stephen is just
	borrowing them and they will be returned in
	good condition.

	Alexa : Try taking Debra's mugs. She'll
	probably be too busy eating to notice you're
	taking them.""",



	"AlexaRepeat":
	"""Alexa : Still being Stephen's errand boy?""",

	"Completion": False
}

stage_6Coffee = {

	"StephenStart":
	"""Stephen : Ah Bruce! Thanks for getting all
	those. In honesty I felt quite bad for making you
	go and fetch all these things after I told you to
	go but you'd already left so I thought I'd just
	let you continue.

	Bruce : You lazy ass! I've gone around getting all
	this stuff for you! What do I get in return?

	Stephen : The finest cup of coffee you've ever had.

	Bruce : Wait what?

	Stephen : The parts. They're to repair one of the
	best coffee machines in the world. The tech that you
	got from the vault was a communicator that was
	perfect size for me to put in this coffee maker so
	that I can make coffee remotely from my phone.

	Bruce : You said a machine.

	Stephen : Coffee MACHINE aaaaand that about does it.
	Good as new. Hand me a mug so we can test it out.

	*Enters Samantha

	Samantha : Good God what is the meaning of this!?

	Bruce : Stephen can explain.

	Stephen : Uh What?

	Samantha : You two were taking so long on getting
	back to me about the mainframe I came here to get
	an update and I see you wasting your time with your
	coffee machine! What have you been doing??

	Stephen : Don't worry. The mainframe is fixed. I
	gave Bruce a USB stick with an executable that fixes
	the mainframe by itself. It should have finished long
	ago so I fixed a machine. Actually I need that USB,
	can you go get it for me Bruce?

	Samantha : Wait one moment! Bruce! Every time you
	spoke to me you said you were fixing the mainframe!
	You lied to me!

	Bruce : In my defence I didn't know what I was doing.

	Samantha : Lousy defence. Go fetch Stephen's USB.

	*Samantha storms out of the lab""",



	"StephenRepeat":
	"""Stephen : Have you got my USB?""",



	"SamanthaRepeat":
	"""Samantha : Don't talk to me you've wasted enough
	time as it is.""",



	"JeniferRepeat":
	"""Jenifer : Still not finished fetching things yet?""",



	"DebraRepeat":
	"""Debra : So what did Stephen need all those parts
	for?

	Bruce : He fixed some fancy coffee machine.

	Debra : Really?

	Bruce : Yeah I kinda spent my whole day doing
	pointless things.

	Debra : Yeah you have. What do I have to do to
	get you to run errands for me?

	Bruce : I won't do errands for you.

	Debra : Aww please?

	Bruce : No.""",



	"BobRepeat":
	"""Bob : How are you still doing errands for Stephen?

	Bruce : I don't know... My job isn't supposed to be
	this...""",



	"AlexaRepeat":
	"""Alexa : You look exhausted. You haven't even done
	anything today. At least you get paid to do nothing.

	Bruce : Yeah I guess...""",

	"Completion": False
}

stage_7Coffee = {

	"StephenStart":
	"""Stephen : Thank you Bruce. You've been really
	helpful today. I haven't even left my seat.

	Bruce : Yeah and I've been up and around everywhere.

	Stephen : Well anyway have a cup of coffee.

	Bruce : It better be good.

	Stephen : I think you'll like it.

	Bruce : So, that piece of tech you put in there.
	Why was it in the vault in the first place?

	Stephen : No idea, but it was a communicator so
	it was probably transmitting information. Possibly
	from a double agent or something.

	Bruce : Shouldn't we alert Samantha or Debra?

	Stephen : Probably, but Samantha is really not going
	to listen to use right now and Debra will have left by
	now.

	Bruce : So we just assume things will be ok?

	Stephen : Yeah why not? I reconfigured the communicator
	so now it only responds and communicates to my phone.
	We have stopped an information leak whilst fixing my
	machine. I think that's worth the effort you went through
	today.

	Bruce : When you put it that way, yeah I guess it was
	worth the effort.

	Stephen : Well now that that's that have some coffee.

	Bruce : Sure thing.

	Stephen : Now about your internet usage. I think you
	need to see someone about your internet usage. I think
	you have a problem...

	Bruce : WHAT DO YOU KNOW?

	Stephen : Too much...""",

	"Completion": False
}

stage_4Agent = {

	"DebraStart":
	"""Bruce : Hi Debra I have something you might want
	to take a look at.

	Debra : Is it more snacks?.

	Bruce : Um no Anyway these logs that printed after
	Stephen's fix worked look wrong. There shouldn't be an
	anonymous log. I think someone is up to something.

	Debra : Hmm it would appear so... We need to have a
	check around and see if there are any suspicious items
	around the place. And by we I mean you because I need
	to stay available to do work in case more comes.

	Bruce : You're making me go round and find items?

	Debra : You're the one with nothing to do at the
	moment so you should do it.

	Bruce : What if something happens. What am i supposed
	to do?

	Debra : Wow you must be a terrible field agent if
	you're asking me that question. You figure out what to
	do. I'll wait for you to return with some evidence.""",



	"DebraRepeat":
	"""Debra : Chop Chop! We, I mean you need to find
	suspicious items!""",



	"StephenRepeat":
	"""Stephen : Thanks for putting the USB in the
	mainframe for me. Don't worry about the reports they
	are boring to go through anyway.""",



	"JeniferRepeat":
	"""Jenifer : Bruce I think Samantha said she wanted
	an update on that mainframe thingy.""",



	"SamanthaStart":
	"""Samantha : How is the mainframe?

	Bruce : It looks like Stephen fixed it.

	Samantha : Good. Stay out of trouble for the rest
	of the day. Go help someone or something I don't have
	anything unimportant for you to screw up.

	Bruce : Ouch way to hurt a man's ego.""",



	"SamanthaRepeat":
	"""Samantha : I told you to go help someone else. I'm
	really busy at the moment.""",



	"BobRepeat":
	"""Bob : You finished helping Stephen yet?

	Bruce : Yeah but I'm helping Debra with something now.

	Bob : Oh ok don't let me stop you then.""",



	"AlexaStart":
	"""Bruce : Alexa have you seen anything suspicious lately?

	Alexa : Your face.

	Bruce : Not funny. Seriously are there any things out of
	place?

	Alexa : Not really. I don't think anything is out of the
	oridnary.

	Bruce : Ok well keep an eye out will you?

	Alexa : Fine I'll keep an eye out. Though...

	Bruce : What?

	Alexa : How long has that camera been there?

	Bruce : I have no idea. I didn't notice it before.

	Alexa : Intersting...""",



	"AlexaRepeat":
	"""Alexa : Sorry Bruce I haven't found anything suspicious looking.""",

	"Completion": False
}

stage_5Agent = {

	"DebraStart":
	"""Debra : Well that's it. Someone must be leaking information to
	external people. It's too obvious. We have to be careful selecting
	who to bring to Samantha. From what we know it has to be someone
	who has enough free time to put these items in their places and
	they have to have access to Samantha's office.

	Bruce : That's not much to go on.

	Debra : You're a smart field agent. Figure it out. I'll keep these
	evidence items safe. Go find the double agent. Try run the
	fingerprints on the gun in the mainframe. Tell me when you have
	found out.""",



	"StephenRepeat":
	"""Stephen : Thanks for helping out earlier. I'm kinda doing some
	important experiments and I need to focus. Sorry I can't chat.""",



	"JeniferRepeat":
	"""Jenifer : Are you still doing errands for people? Can you take
	my trash out for me please?

	Bruce : No I'm not a dustbin man.""",



	"DebraRepeat":
	"""Debra : Who have you deduced that the double agent is?

	Bruce : I don't know yet.

	Debra : Well hurry up. We have no idea what they could be doing
	whilst we are making our move.""",



	"SamanthaRepeat":
	"""Samantha : I told you to go help someone else. I'm
	really busy at the moment.""",


	"BobRepeat":
	"""Bob : You finished helping Stephen yet?

	Bruce : Yeah but I'm helping Debra with something now.

	Bob : Oh ok don't let me stop you then.""",



	"AlexaRepeat":
	"""Alexa : Sorry Bruce I haven't found anything suspicious looking.""",

	"Completion": False
}

stage_6Agent = {

	"SamanthaStart":
	"""Samantha : I can't believe it. No wonder the mixing up of the names
	felt dodgy. You were planted here. By whom? I demand answers!

	Jenifer : You won't find anything out.

	Samantha : I hate to sound cliche but we have ways of making you talk.

	Bruce : Are we going to torture her!? I miss this so much!

	Samantha : Yes Bruce for once we're going with something you like and
	I hope you're performance is good.

	Bruce : I always have good performance.

	*click
	*A bright flash filled the room accompanied by a deafening sound

	Bruce : AARG What was that?

	Samantha : What?

	Bruce : It must have been a flash grenade!

	Samantha : What?

	Bruce : You can't hear?

	Samantha : This is not the time for a can of beer now chase after her!

	*Bruce chases Jenifer out of the reception and to the stairs

	Bruce : She's going down the stairs!

	Samantha : Shoot her!

	Do you shoot her chest or her leg?""",

	"Completion": False
}

stage_7Agent = {

	"SamanthaStart":
	"""Samantha : Damm it Bruce you shot her in the chest. Look at her
	she's going to bleed out!

	Bruce : Yeah that is a lot of blood for such a small woman...

	Samantha : Well thanks a lot. Now we can't interrogate her to find
	out who put her here.

	Bruce : Should we try and save her?

	Samantha : Oh what's the point. She's practically unsavable now and
	now she's gone we won't have an information leak. Just clean up
	blood and dump her body somewhere.

	Bruce : Wow you are cold.

	Samantha : I would have thought you'd have better field agent
	judgement and that you'd have shot her leg. You're on suspension for
	a week to re-collect your thoughts. When you're back I want you sharp
	and on edge for anything.

	Bruce : Ok.

	*Alexa enters

	Alexa : Bruce are you ok?

	Bruce : Yeah I'm fine but I'm also on suspension.

	Alexa : How come?

	Bruce : For shooting a double agent.

	Alexa : I just don't get Samantha sometimes...""",

	"Completion": False
}

stage_8Agent = {

	"SamanthaStart":
	"""Samantha : Good shot Bruce!

	Bruce : Thanks.

	Samantha : Perfet. The bullet hasn't gone through the leg because it
	got caught on the shin bone. I'll see to it personally that Jenifer
	spills the secrets. Maybe a knife pressing into this open wound would
	persuade her to talk.

	Bruce : Uhh since when have you been into this sort of thing?

	Samantha : Oh I was very good at interrogating from when I used to be
	a field agent too.

	Bruce : Shouldn't you check her in case she has another grenade?

	Samantha : If she does she still won't be able to flee and that
	definitely means she won't use a lethal one if she has one.

	Bruce : Ok so all is under control.

	Samantha : Yes. Thanks to you we can find out who sent her and we can
	give them a little piece of our mind. Get something to clean up the
	blood stains.

	Bruce : So I guess something interesting did eventually happen today.
	Shame I'm still doing errands for people.""",

	"Completion": False
}

stage_9Agent = {

	"SamanthaStart":
	"""Samantha : This is outrageous Bruce!

	Bruce : I can explain!

	Samantha : You've wasted your time, you've wasted my time and you've
	wasted other people's time!

	Bruce : But there is an explanation for all this!

	Samantha : An explanation for you going all over taking things and
	harrassing co-workers?

	Burce : Um... Yes?

	Samantha : That just does it. You've really shown me that you can't be
	trusted. Even here! I'm putting you under Suspension for wasting time
	and resources. Report back here in a week.

	Bruce : Come on you haven't even given me a chance to explain myself.

	Samantha : You don't have to! OUT!""",

	"Completion": False
}